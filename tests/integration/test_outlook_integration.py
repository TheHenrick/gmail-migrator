...
class TestOutlookIntegration:
    ...
    def test_full_migration_flow(
        ...
        # Mock the create_folder response
        mock_outlook_client.create_folder.return_value = {
            "id": "new_folder",
            "displayName": "Migrated Emails",
            "parentFolderId": None,
            "childFolderCount": 0,
            "totalItemCount": 0,
            "unreadItemCount": 0,
        ...
        assert auth_url_response.json() == {"auth_url": "https://login.microsoftonline.com/auth"}

        # Step 2: Create a folder in Outlook
        folder_response = test_client.post("/outlook/folders?name=Migrated%20Emails")
        assert folder_response.status_code == 200
        folder = folder_response.json()
        assert folder["display_name"] == "Migrated Emails"  # Match the mock response
        folder_id = folder["id"]

        # Step 3: Migrate a single email
        mock_gmail_client.get_email.return_value = {
            "id": "test_email",
            "subject": "Test Subject",
            "has_attachments": False,
        }

        mock_outlook_client.import_email.return_value = {
            "id": "outlook_email_id",
            "subject": "Test Subject",
        }

        migrate_response = test_client.post(f"/outlook/migrate-email?email_id=test_email&folder_id={folder_id}")
    ...
    def test_auth_and_folder_management(self, mock_client_class, mock_oauth_flow, test_client):
        ...
        create_folder_response = test_client.post("/outlook/folders?name=New%20Folder", headers=headers)
